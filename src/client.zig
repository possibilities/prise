...
pub const ClientState = struct {
    ...
    pub const RequestInfo = union(enum) {
        spawn: struct { cwd: ?[]const u8 = null, cmd: ?[]const u8 = null, old_pty_id: ?u32 = null },
...
pub const App = struct {
    ...
    fn onPipeRead(l: *io.Loop, completion: io.Completion) anyerror!void {
        ...
        // switch must win over an incidental empty-state force-quit (the
        // keep_attached path races these two against each other). A
        // deferred frame request is drained last so any render lands
        // against the already-reconciled app state. See PR #112 and
        // detachIfPending for the re-entrancy hazard these flags
        // exist to avoid.
    ...
    fn onRecv(l: *io.Loop, completion: io.Completion) anyerror!void {
        ...
        switch (completion.result) {
            .recv => |initial_bytes_read| {
                    ...
                    while (app.msg_buffer.items.len > 0) {
                        ...
                        switch (action) {
                            ...
                            .pty_exited => |info| {
                                ...
                                const no_surfaces = removed_surface and app.surfaces.count() == 0;
                ...
                // remain onPipeRead-only. drainPendingFrameRequest lands
                // after so any frame queued by a handler paints against
                // the already-reconciled session state.
                app.switchSessionIfPending();
                app.drainPendingFrameRequest();
    ...
    pub fn spawnPty(self: *App, opts: UI.SpawnOptions) !void {
        ...
        try self.state.pending_requests.put(msgid, .{ .spawn = .{ .cwd = cwd, .cmd = opts.cmd } });
        ...
        var idx: usize = 4;
        if (cwd) |resolved_cwd| {
            map_items[idx] = .{ .key = .{ .string = "cwd" }, .value = .{ .string = resolved_cwd } };
            idx += 1;
        }
        if (opts.cmd) |cmd| {
    ...
    pub fn switchToSession(self: *App, target_session: []const u8) !void {
        ...
        // Consume session_ending atomically: if the current session is being
        // torn down (keep_attached last-pane path), the Lua side has already
        // called prise.delete_session on it, so saving below would re-create
        // the phantom file. Clear the flag in the same check to keep its
        // lifetime scoped to a single switch — set-and-consume in one
